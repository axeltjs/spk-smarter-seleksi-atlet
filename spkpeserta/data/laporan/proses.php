...
if(isset($_SESSION['username']) AND isset($_SESSION['password'])){
	...
	$item=mysqli_query($koneksi,"SELECT * FROM tjadwal,tlapangan,tjam WHERE tjadwal.kdLapangan=tlapangan.kdLapangan 
						   AND tjadwal.kdJam=tjam.kdJam AND tjadwal.kdJadwal='$_GET[id]'");
	$_data=mysqli_fetch_array($item);
	
	$SQL=mysqli_query($koneksi,"SELECT * FROM tboking_temp WHERE jamBokingTemp='$_data[jam]'");
   ...
   <script language='javascript'>
	...
	window.location=('../../frame.php?load=boking&action=input')
		...
		mysqli_query($koneksi,"DELETE FROM tboking_temp WHERE kdbokingTemp=$_GET[id]")or die (mysql_error());
	...
	";
		...
		mysqli_query($koneksi,"UPDATE tboking SET statusBayar='$_POST[rbStatus]' WHERE kdBoking='$_POST[id]'");
	echo"
	<script language='javascript'>
...
$lunas="B";
$tot=mysqli_query($koneksi,"SELECT SUM(subTotalTemp) AS totalBayar FROM tboking_temp WHERE idSession='$_SESSION[username]'");
